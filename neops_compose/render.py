...
from neops_compose.ports import DEFAULT_MONITOR_PORT, MONITOR_CONTAINER_PORT
...
# The compose fragment compose.traefik-shared-host.yaml extends its traefik service from.
TRAEFIK_PORTS_FRAGMENT = "traefik/ports.yaml"
...
def render(env: Env, scenario: Scenario, paths: Paths) -> list[Path]:
    ...
    emit("monitor.env", monitor_env(env))
    ...
    if scenario.shared_host:
        emit(TRAEFIK_PORTS_FRAGMENT, _yaml_json(traefik_ports(env, scenario)))
...
def monitor_env(env: Env) -> str:
    """The monitor app is published under whatever path NEOPS_WORKFLOWS_URL carries, and the
    proxy strips that prefix: MONITOR_BASE_PATH tells the app so its HTML can carry it."""
    base_path = PublicUrl.parse(env.require("NEOPS_WORKFLOWS_URL")).path
    return f"# Generated by ./neops render from .env. Do not edit.\nMONITOR_BASE_PATH={base_path}\n"


def traefik_ports(env: Env, scenario: Scenario) -> dict:
    """Shared-host mode publishes NEOPS_MONITOR_PORT only while the monitor is its own
    entrypoint; compose cannot make a port mapping conditional, so the overlay extends this."""
    traefik: dict = {}
    if monitor_entrypoint_wanted(public_urls(env, scenario), scenario):
        port = env.get("NEOPS_MONITOR_PORT", str(DEFAULT_MONITOR_PORT))
        traefik["ports"] = [f"{port}:{MONITOR_CONTAINER_PORT}"]
    return {"services": {"traefik": traefik}}
