...
without TLS). `NEOPS_ENGINE_URL`, `NEOPS_KEYCLOAK_URL` and `NEOPS_GRAFANA_URL` may not carry a
path in this mode — a path only makes sense once you route by prefix, which is what the
shared-hostname overlay is for.
...
`metrics/grafana`). Only Grafana is ever routed publicly, and only when `NEOPS_GRAFANA_URL` is
set — leave it unset to keep Grafana reachable at `127.0.0.1:3000` only (external-proxy mode) or
unrouted (Traefik mode). Set `NEOPS_GRAFANA_ADMIN_PASSWORD` either way; Grafana needs it to start.
